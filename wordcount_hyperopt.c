...
// gcc -O3 -march=znver5 -mtune=znver5 -flto -fomit-frame-pointer -funroll-loops
...
void debug_dump_stats()
{
    ...
    DBG("Memory: allocated=%lu freed=%lu leaked=%lu",
        memory_allocated,
        memory_freed,
    ...
    DBG("Run-length histogram (tokens): [1]=%lu [2-3]=%lu [4-7]=%lu [8-15]=%lu "
        "[16-31]=%lu [32-63]=%lu [64-127]=%lu [128+]=%lu",
        word_len_hist[0],
        word_len_hist[1],
        word_len_hist[2],
        word_len_hist[3],
    ...
    for (int i = 0; i < 16; i++)
    {
        if (thread_word_counts[i] > 0)
        {
            ...
            DBG("T%02d: %.3fms %lu words (%lu unique) %lu collisions (avg "
                ...
                i,
                thread_times[i],
                thread_word_counts[i],
                thread_unique_counts[i],
                collision_counts[i],
                collision_counts[i] > 0
                        ? (double)probe_lengths[i] / collision_counts[i]
                        : 0.0);
        }
    }
...
static void discover_vcache_cpus()
{
    ...
    for (int cpu = 0; cpu < ncpus; cpu++)
    {
        ...
        size_t val = 0;
        char unit = 0;
        if (sscanf(sizebuf, "%zu%c", &val, &unit) != 2)
            continue;
        size_t l3_bytes = val;
        if (unit == 'K' || unit == 'k')
            l3_bytes <<= 10;
...
static void table_grow(ThreadTable *t)
{
    ...
    table_grow_ns_total +=
            (uint64_t)(_tg2.tv_sec - _tg1.tv_sec) * 1000000000ull +
    ...
    for (size_t i = 0; i < size; i++)
    {
        ...
        if (c >= 0x80)
        {
            ...
            if ((c & 0xE0) == 0xC0)
                ...
                utf8_sequences_3byte++;
            else if ((c & 0xF8) == 0xF0)
                utf8_sequences_4byte++;
            else
                utf8_errors++;
#endif
...
static void *worker(void *arg)
{
    ...
    if (thread_cpu_actual)
        thread_cpu_actual[unit->thread_id] = running_cpu;
    DBG("Worker T%02d: target cpu=%d running cpu=%d",
        unit->thread_id,
        cpu_id,
        running_cpu);
#endif
...
#endif
    process_chunk_avx512(unit->table,
                         unit->data + unit->start,
                         unit->end - unit->start,
                         unit->drop_leading);
#ifdef DEBUG
...
static void heap_sift_down(MinHeap *h, int i)
{
    for (;;)
    {
        ...
        if (l < h->size && h->entries[l].count < h->entries[s].count)
            s = l;
        if (r < h->size && h->entries[r].count < h->entries[s].count)
            s = r;
        if (s == i)
            break;
...
int main(int argc, char *argv[])
{
    ...
    for (int i = 0; i < NUM_THREADS; i++)
    {
        ...
        tables[i].entries = MEMORY_ALLOC_ALIGNED(
                CACHELINE, tables[i].capacity * sizeof(Entry));
        memset(tables[i].entries, 0, tables[i].capacity * sizeof(Entry));
        tables[i].string_pool =
                MEMORY_ALLOC_ALIGNED(CACHELINE, STRING_POOL_SIZE);
        ...
        madvise(tables[i].entries,
                tables[i].capacity * sizeof(Entry),
                MADV_HUGEPAGE);
